...
<div class="justify-center items-center flex flex-col mx-auto mt-16">
    <form on:submit|preventDefault={selectedCategory} class="mt-8 flex flex-col items-start lg:w-[30%] px-6 lg:px-0">
        ...
        <label class="flex items-center w-full mx-auto"><input type="checkbox" class="mr-4" bind:checked={fullStatRestrictions} disabled={statRestrictions}><p>Full Stat restrictions (include Vigor, Mind, etc.)</p></label>
    ...
    <h2 class="lg:w-3/5 flex text-left items-start text-4xl font-extrabold mt-4 mb-8 2xl:mt-12 ">Run information</h2>
        ...
        <div id="buttoncontainer" class="flex justify-end w-3/5 mt-4 items-center">
            <div class="flex text-base font-semibold items-center justify-center inline-block px-6 py-4 leading-none border rounded text-[#000] border-[#000] hover:border-[#105D97] hover:text-[#105D97] hover:bg-[#fff] mt-4 ">
                <a href="/submit?Game=eldenring&{submissionstring}"><button type="submit" name="challengeSubmission" value={arr}>Submit This Challenge</button></a>
            </div>
        </div>  
        {/if}
</div>
